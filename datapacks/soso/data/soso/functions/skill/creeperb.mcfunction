execute unless entity @e[tag=creeper,type=creeper] as @a[tag=creeper] at @s run summon minecraft:creeper ~ ~ ~ {Attributes:[{Name:"generic.attackDamage",Base:1}],Tags:["creeper"],Team:"Rg",ignited:1,PortalCooldown:30}
...
execute at @e[tag=creeper,nbt={PortalCooldown:1}] as @a store result entity @s[distance=..7] Inventory[{Slot:-106b}].tag{Nonknow:1b}.AttributeModifiers[{AttributeName:"generic.knockbackResistance"}].Amount double 1 run scoreboard players set @s zTEST 1

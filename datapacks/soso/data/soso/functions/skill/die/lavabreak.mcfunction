...
execute if score dieskill zTEST matches 1 run execute as @a[team=Gc,distance=..7] store result score @s zTEST run data get entity @s Inventory[].tag{Nonknow:1b}.AttributeModifiers[{AttributeName:"generic.knockbackResistance"}].Amount 10
execute if score dieskill zTEST matches 1 run execute as @a[team=Gc,distance=..7] store result entity @s Inventory[].tag{Nonknow:1b}.AttributeModifiers[{AttributeName:"generic.knockbackResistance"}].Amount double 0.1 run scoreboard players operation @s zTEST /= 2 zTEST

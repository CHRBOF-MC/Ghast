...
execute as @s[scores={ramdon=2}] run execute store result entity @e[limit=1,tag="寒冰聖獸"] Health double 0.1 run scoreboard players add 寒冰聖獸 skyway 50
execute as @s[scores={ramdon=3}] run tellraw @a ["",{"text":"藍隊完成委託紅隊神獸血量降低50點。","color":"blue","bold":true}]
execute as @s[scores={ramdon=3}] run execute store result entity @e[limit=1,tag="烈火聖獸"] Health double 0.1 run scoreboard players remove 烈火聖獸 skyway 50
...
execute as @s[scores={ramdon=10}] run scoreboard players operation @e[team=RED,tag="戰場遺願"] BUY /= 2 zTEST
...
execute as @s[scores={ramdon=16}] run give @a[team=BLUE] minecraft:golden_apple[custom_name="金蘋果",lore=[{"text":"魔法般的食物"},{"text":"可以快速恢復血量"}]] 5
